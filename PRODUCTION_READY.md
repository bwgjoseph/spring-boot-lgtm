...
## 4. Logging Strategy: Labels, Text & Structured Metadata
Loki 3.0+ leverages structured metadata to efficiently correlate data without crashing memory.

| Tier | Storage Type | Best For |
| :--- | :--- | :--- |
| **Labels** | Indexed (Highly searchable) | Filtering large volumes (e.g., `service_name`, `env`). |
| **Structured Metadata** | Non-indexed Fields | Correlation (e.g., `trace_id`, `span_id`) without high memory cost. |
| **Log Line** | Raw Text | Human readability. |

*   **Recommendation:** Use structured metadata for high-cardinality data like `trace_id`. Grafana can automatically detect these fields, enabling the "View Trace" button in Loki without custom regex.

## 5. Storage Strategy: MinIO (Recommended)
Since local clusters often rely on **ReadWriteOnce (RWO)** PVCs, **MinIO** is essential as an S3-compatible middle layer.
*   **Architecture:** `Loki/Tempo (S3 API) -> MinIO (Pod) -> PVC (Disk)`.
*   **Benefit:** This provides the "Shared Storage" needed for distributed components (Loki/Tempo) without requiring an complex RWX filesystem (NFS/Ceph).
*   **Performance:** Back MinIO with high-performance SSD/NVMe storage classes for optimal results.

## 6. Coordination: Memberlist (Gossip)
If you scale the stack beyond a single binary, Simple Scalable mode uses the **Memberlist** protocol for coordination.
*   **How it works:** Pods "gossip" with each other to maintain a shared "Hash Ring," which tracks responsibility for log/trace chunks.
*   **On-Prem Note:** Ensure port `7946` is open between all observability pods to allow the gossip protocol to function.
