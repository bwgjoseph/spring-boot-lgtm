...
helm upgrade --install tempo grafana-community/tempo -n monitoring --version 1.10.15

# Install or Upgrade Loki (Logs)
helm upgrade --install loki grafana/loki -n monitoring -f deployment/values-loki.yaml --version 6.16.0

# Install or Upgrade Prometheus (Metrics)
helm upgrade --install prometheus prometheus-community/prometheus -n monitoring -f deployment/values-prometheus.yaml --version 25.27.0

# Install or Upgrade Grafana (Visualization)
helm upgrade --install grafana grafana-community/grafana -n monitoring -f deployment/values-grafana.yaml --version 8.5.12
```

---

## 2. Deploying/Upgrading Grafana Alloy
Alloy acts as the central gateway for all observability data.

### Fresh Install or Upgrade
The command is the same for both. Helm will automatically detect if it needs to install or update the existing release.

```bash
helm upgrade --install alloy grafana/alloy -n monitoring -f deployment/values-alloy.yaml --version 0.6.1
```
...
```bash
# Forward the Alloy daemonset pod (adjust name if needed or use label)
kubectl port-forward ds/alloy 12345:12345 -n monitoring
```
...
- **Service Graph:** Automatically generated in the Tempo "Service Graph" tab.
